...
public class FrameCommand extends ListenerAdapter
{
	...
	private String files[] = {"HAZBIN HOTEL (PILOT) .mp4", "HELLUVA BOSS - C.H.E.R.U.B (S1)： Episode 4 .mp4", "HELLUVA BOSS - EXES AND OOHS (S2)： Episode 3.mp4"
	...
	public FrameCommand(ExecutorService executor) 
	{
		...
		if(event.getName().equals("frame")) 
		{
			this.executor.submit( () -> 
			{
				try 
				{
					...
					event.getHook().sendMessageEmbeds(builder.build()).addFiles(upload).queue();
					file.delete(); 
				}
